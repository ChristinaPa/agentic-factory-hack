...
// Maps a diagnosed fault type (e.g. "curing_temperature_excessive") to the
// skills a technician needs and the part numbers that are typically required.
//
// These mappings are intentionally hardcoded for the workshop:
//   - they ground the LLM with a verified shortlist (no hallucinated parts)
//   - they keep the agent fast and deterministic
...
public sealed class FaultMappingService : IFaultMappingService
{
    ...
    private static readonly IReadOnlyList<string> NoParts = Array.Empty<string>();
    ...
    public IReadOnlyList<string> GetRequiredSkills(string faultType)
    {
        ...
        // TryGetValue returns false for unknown fault types -> fall back to default.
    ...
    public IReadOnlyList<string> GetRequiredParts(string faultType)
    {
        ...
        return FaultToParts.TryGetValue(faultType, out var parts) ? parts : NoParts;
    }
}
